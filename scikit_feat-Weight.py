...
def main():
    ...
    baseline_model= ScikitFeatBaseline_Model(device=device, input_dim=data.X.shape[1], classes=classes, dataset=data.dataset)
    model= Models.Univariate_IELayer(device=device, input_dim=data.X.shape[1], model=baseline_model)
    model.to(device)
    ...
    dir_path=default_path+"Weight/"
    if not os.path.exists(dir_path):
             os.makedirs(dir_path)

    pickle.dump( model.return_layer_weights(), open(dir_path+feat+f"-w-{config['epoch']}.pkl", "wb") )
...
if __name__=="__main__":
    ...
    feat= "scikit_feat-Weight-"+args.data
    ...
    model= main()
    post(default_path, config, feat, model, data.y_val, classes, val_dataloader, "Weight")
