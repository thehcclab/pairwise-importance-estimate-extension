...
def main():
    ...
    model= DryBeanBaseline_Model(device=device, input_dim=np.count_nonzero(weights), classes=data.classes)
    ...
    return args, model, data, val_dataloader
...
if __name__=="__main__":
    ...
    w_path={
        "DF":default_path+f"DF/DryBean-DF-w-{config['epoch']}.pkl",
        "Weight":default_path+f"Weight/DryBean-Weight-w-{config['epoch']}.pkl",
        "Fisher":default_path+f"Fisher/fisher_score_idx.pkl",
        "FScore":default_path+f"FScore/fscore_idx.pkl",
        "ThresholdedWeight":default_path+f"Weight/DryBean-ThresholdedWeight-w-{config['epoch']}.pkl",
        "Grad-ROC": default_path+f"Grad/list/DryBean-Grad-list-{config['epoch']}.pkl",
        "Grad-AUC": default_path+f"Grad/list/DryBean-Grad-list-{config['epoch']}.pkl",
        "Grad-COV": default_path+f"Grad/list/DryBean-Grad-list-{config['epoch']}.pkl",
        "Grad-STD": default_path+f"Grad/list/DryBean-Grad-list-{config['epoch']}.pkl",
        "NFS":default_path+f"NFS/DryBean-NFS-w-{config['epoch']}.pkl"
    }

    feat= f"DryBean-subset-{args.model}-{args.subset}"
    load_w=w_path[args.model]

    args, model, data, val_dataloader= main()
