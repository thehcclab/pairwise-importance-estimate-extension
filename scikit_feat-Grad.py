...
def main():
    ...
    model= Models.Univariate_IEGradient(device=device, input_dim=data.X.shape[1], model=baseline_model)
    model.to(device)
    ...
    pickle.dump( model.return_IE_grad(), open(default_path+"Grad/list/"+save_name+"-"+str(config["epoch"])+".pkl", "wb") )
...
if __name__=="__main__":
    ...
    feat= "scikit_feat-Grad-"+args.data
    save_name= "scikit_feat-Grad-list-"+args.data
