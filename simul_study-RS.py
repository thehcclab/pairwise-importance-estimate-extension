...
def main():
    ...
    array= rs_init(f"simul_study/", args.rs, args.percentile, data.X)
    ...
    model= SimulationStudyBaseline_Model(device=device, input_dim=np.count_nonzero(array), classes=data.classes)
    #model= Models.Univariate_RS(device=device, input_dim=data.X.shape[1], model=baseline_model,rs_path="simul_study/",rs_selection=args.rs).to(device)
    model.to(device)
...
if __name__=="__main__":
    ...
    feat= f"simul_study-subset-RS{args.rs}-{args.percentile}"

    default_path= init(args, default_path, config)
    model, classes, data, val_dataloader= main()
    post(default_path, config, feat, model, data.y_val, classes, val_dataloader, f"RS/RS{args.rs}")
