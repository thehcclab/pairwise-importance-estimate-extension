...
def main():
    ...
    model= Models.Univariate_IELayer(device=device, input_dim=data.X.shape[1], model=baseline_model).to(device)
    ...
    pickle.dump( model.return_layer_weights(), open(f"{dir_path}{feat}-w-{config['epoch']}.pkl", "wb") )
