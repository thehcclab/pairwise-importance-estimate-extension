...
def main():
    ...
    model= Models.Univariate_IELayer(device=device, input_dim=dataset_val.X.shape[1], model=baseline_model).to(device)
    ...
    pickle.dump( model.return_layer_weights(), open(dir_path+feat+"-w-"+str(config["epoch"])+".pkl", "wb") )
...
if __name__=="__main__":
    ...
    model= main()
    post(default_path, config, feat, model, dataset_val.y, classes, val_dataloader, "Weight")
